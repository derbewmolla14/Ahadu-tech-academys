...
export default function Register() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setSuccess('Registration submitted successfully. Please wait for approval and then login.');
      setEmail('');
      setPassword('');
    ...
    <div className="min-h-screen bg-slate-950 px-4 py-12 text-white sm:px-6">
      <div className="mx-auto flex w-full max-w-3xl flex-col gap-8">
        <div className="rounded-3xl border border-slate-800 bg-slate-900 px-8 py-6 shadow-xl shadow-slate-950/50">
          ...
          <p className="mt-3 max-w-xl text-slate-400">Register as a student to access content after approval.</p>
        </div>

        <div className="rounded-3xl border border-slate-800 bg-slate-900 px-8 py-10 shadow-xl shadow-slate-950/50">
          {success ? <div className="mb-6 rounded-2xl bg-emerald-500/10 px-4 py-3 text-emerald-200">{success}</div> : null}
          ...
          <div className="mt-6 text-sm text-slate-400">
            Already have an account? <button type="button" onClick={() => navigate('/login')} className="text-white underline">Login here</button>.
          </div>
        </div>
      </div>
    </div>
